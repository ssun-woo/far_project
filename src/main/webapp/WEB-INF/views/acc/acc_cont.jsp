		...
		<div class="shop_cont">
			<div class="shop_name">
				...
				<label class="jjim_shop"> <c:if test="${JJim==0}">
						<form
							action="/acc/cont/jjim?detail_cate=${cate}&store_num=${store_num}"
						...
						<form
							action="/acc/cont/jjim_del?detail_cate=${cate}&store_num=${store_num}"
				...
				</label> <input type="hidden" id="memId" value="${memId}">
			</div>
			<hr>
				...
				<div class="shop_info">
					...
					<ul class="info_cont">
						...
						<li><p>
								...
								<a href="#target" onclick="review_count_page_btn()">${review_count}개</a>의
								상품평
							</p></li>
						<li><p>
								${s.storeAddr1} ${s.storeAddr2 }<br> <a href="#target"
									onclick="map_page_btn()">지도보기</a>
			...
			<div class="shop_info_cont">
				<input type="radio" id="service" name="show" value="1" checked /> <input
					...
					type="radio" id="map_guide" name="show" value="4" /> <input
					type="radio" id="review" name="show" value="5" />


				<div class="tab">
					<label for="service">상품</label> <label for="information">정보</label>
					<label for="cancellation">이용안내 / 취소규정</label> <label
						for="map_guide">지도</label> <label for="review">이용후기</label>
				</div>
				...
				<div id="information_content" style="display: none;"
					...
					<div class="acc_policy">
						...
						<ul>
							...
							<li>&#45; 레이트 체크아웃의 경우 오후 3시까지 Half-day 비용이 발생합니다. (객실타입마다
								금액상이)</li>
							...
							<li>&#45; 엑스트라베드 : 추가가능, 1박 38,500원 현장결제 (단 Studio, 원베드 트윈에는
								제공이 되지 않습니다.)</li>
							<li>&#45; 침구 : 추가가능, 1채 1박당 22,000원 현장결제</li>
							<li>&#45; 아기침대(만1세까지) : 1베드 룸타입에만 가능/ 체크인시 잔여수량 있을 시
								가능(사전요청불가)</li>
							<li>&#45; 트윈 : 더블베드+싱글베드</li>
						</ul>
					</div>
				...
				<div id="cancellation_content" style="display: none;"
					...
					<div class="cancellation_policy">
						...
						<ul>
							...
							<li>상품별, 숙소별 별도 취소 규정이 적용되니 예약진행 시 이용동의란의 취소가능일을 반드시 확인해 주시기
								바랍니다.</li>
							<li><br></li>
							<li>실시간 예약의 특성상 동일한 객실에 중복 예약이 발생할 수 있으며, 이 경우 먼저 결제완료된 예약
								건에 우선권이 있으므로 이후 중복 예약 건은 취소될 수 있습니다.</li>
							<li><br></li>
					...
					<div class="refund_process">
						...
						<ul>
							...
							<li>&#45; 신용카드/체크카드 : 취소 완료 후 3~5 영업일 이후 환불(승인/매입 구분 불가)
								됩니다.</li>
							...
							<li>&#45; 패널티는 포인트에서 선차감되어 환불되며, 패널티 금액이 포인트 사용 금액보다 높을 경우
								잔여 패널티 금액은 결제하신 결제수단(신용카드 등)에서 차감 후 환불됩니다. 단, 패널티 금액이 신용카드
								결제금액보다 높을 경우에는 고객센터로 문의 하시기 바랍니다.</li>
						</ul>
					</div>
				...
				<div id="review_content" style="display: none;"
					...
					<div class="Review_cont">
						...
						<div class="review_rating_form">
							...
							<form name="review"
								action="/acc/cont/review?detail_cate=${cate}&store_num=${store_num}"
								...
								<table class="review_write">
									...
									<tr>
										...
										<td colspan="2"><div class="mb-3" id="reviewRating">
												<fieldset>
													<span>별점을 선택해주세요</span> <input type="radio"
														name="reviewRating" value="5" id="rate1"> <label
														for="rate1">★</label> <input type="radio"
														name="reviewRating" value="4" id="rate2"> <label
														for="rate2">★</label> <input type="radio"
														name="reviewRating" value="3" id="rate3"> <label
														for="rate3">★</label> <input type="radio"
														name="reviewRating" value="2" id="rate4"> <label
														for="rate4">★</label> <input type="radio"
														name="reviewRating" value="1" id="rate5"> <label
														for="rate5">★</label>
												</fieldset>
											</div></td>
									</tr>
									...
									<tr>
										<td><input type="submit" value="등록"
											onclick="submit_check();  review_check();"
											class="ReviewWriteBtn"></td>
									</tr>
								...
								<c:forEach var="review" items="${reviewList}">
									<div class="detail_review_table_div">
										<table class="detail_review_table">
											...
											<tr>
												...
												<td style="text-align: left;">${review.memId}<label
													style="color: gray;">| ${review.reviewDate}</label></td>
												<td style="text-align: left;">평점:
													${review.reviewRating}</td>
											</tr>
											<tr>
										...
										<div class="review_buttons">
											...
											<label class="review_recommend"> <c:if
													test="${recommendCheck==null}">
													<form
														action="/acc/cont/recommend?detail_cate=${detailCate}&store_num=${store_num}&reviewNum=${review.reviewNum}"
														method="post" id="recommendBtn" name="reviewRecommend">
														<button type="submit" name="recommend"
															id="reviewRecommend"
															onclick="recommend_check(); updateLike(); return false;">
															<img src="../images/main/review_recommend.png">
															<p id="recommend_count">${review.reviewRecommend}</p>
														</button>

													</form>
												</c:if> <c:if test="${recommendCheck==1}">
													<form
														action="/acc/cont/recommend_del?detail_cate=${cate}&store_num=${store_num}&reviewNum=${review.reviewNum}"
														method="post">
														<button type="submit" name="recommend" id="yes_re"
															onclick="recommend_del_check()">
											...
											<c:if test="${review.memId == memId}">
												...
												<div class="review_edit_del">
													...
													<form
														action="/acc/cont/edit?detail_cate=${cate}&store_num=${store_num}&reviewNum=${review.reviewNum}"
														method="POST">
													...
													<form
														action="/acc/cont/delete?detail_cate=${cate}&store_num=${store_num}&reviewNum=${review.reviewNum}"
														method="POST">
														<input type="hidden" name="reviewNum"
															value="${review.reviewNum}">
														<button type="submit"
												...
												$(document)
														.on(
																...
																function(e) {
																	...
																	window
																			.open(
																					"/acc/cont/edit?detail_cate=${cate}&store_num=${store_num}&reviewNum="
																							+ reviewNum,
																					"update",
																	...
																	var editedReviewContent = document
																			.getElementById("reviewCont").value;
   ...
   h4Element.textContent = '체크인' +  check_in_h + ':' + check_in_m + '체크아웃' +  check_out_h + ':' + check_out_m;
	...
	<script>
		...
		function review_count_page_btn() {
			...
			document.querySelectorAll('#service_content').forEach(
					function(content) {
						content.style.display = "none";
					});
			document.querySelectorAll('#information_content').forEach(
					function(content) {
						content.style.display = "none";
					});

			document.querySelectorAll('#cancellation_content').forEach(
					function(content) {
						content.style.display = "none";
					});

			document.querySelectorAll('#map_content').forEach(
					function(content) {
						content.style.display = "none";
					});
		...
		function map_page_btn() {
			...
			document.querySelectorAll('#review_content').forEach(
					function(content) {
						content.style.display = "none";
					});
	...
	<script type="text/javascript"
		src="//dapi.kakao.com/v2/maps/sdk.js?appkey=a88d036132dec983608208b58361c621a88d036132dec983608208b58361c621"
		async></script>
	<script>
	...
	<script>
		...
		function loadPayPage3() {
			...
			xhttp.open("GET", "/acc/reload_menu?date=" + dateValue
					+ "&totalCount=" + totalCount + "&store_num=" + store_num,
					true);
			xhttp.send();
		}
